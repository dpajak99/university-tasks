...
String formatBytes(int size, [int fractionDigits = 2]) {
  ...
  final int multiple = (log(size) / log(1024)).floor();
  return '${(size / pow(1024, multiple)).toStringAsFixed(fractionDigits)} ${<String>['B', 'kB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'][multiple]}';
}

/// Returns user avatar and name color based on the ID
Color getUserAvatarNameColor(PublicUserData user, List<Color> colors) =>
    colors[user.id.hashCode % colors.length];

/// Returns user initials (can have only first letter of firstName/lastName or both)
String getUserInitials(PublicUserData user) {
  String initials = '';
...
String getUserName(PublicUserData user) =>
...
String getVerboseDateTimeRepresentation(
  ...
  final String formattedTime = timeFormat != null
...
List<Object> calculateChatMessages(
  ...
  final List<Object> chatMessages = <Object>[];
  final List<PreviewImage> gallery = <PreviewImage>[];

  bool shouldShowName = false;

  for (int i = messages.length - 1; i >= 0; i--) {
    final bool isFirst = i == messages.length - 1;
    final bool isLast = i == 0;
    final ChatMessage message = messages[i];
    final bool messageHasCreatedAt = message.createdAt != null;
    final ChatMessage? nextMessage = isLast ? null : messages[i - 1];
    final bool nextMessageHasCreatedAt = nextMessage?.createdAt != null;
    final bool nextMessageSameAuthor = message.author.id == nextMessage?.author.id;
    final bool notMyMessage = message.author.id != user.id;
    ...
    if (showUserNames) {
      final ChatMessage? previousMessage = isFirst ? null : messages[i + 1];

      final bool isFirstInGroup = notMyMessage &&
    ...
    //   if (kIsWeb) {
    //     if (message.imageId.startsWith('http') || message.imageId.startsWith('blob')) {
    //       gallery.add(PreviewImage(id: message.id, uri: message.imageId));
    //     }
    //   } else {
    //     gallery.add(PreviewImage(id: message.id, uri: message.imageId));
    //   }
  ...
  return <Object>[chatMessages, gallery];
}